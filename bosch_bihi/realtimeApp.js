...
var redrawChart = function(targetID, newdata) {
	...
	//Update data labels
	chartRow.select(".label_dchart").transition()
	  .duration(300)
	  .attr("opacity",1)
...
// 		  this.textContent = Math.round(i(t));
			this.textContent = d3.format(".4g")(i(t));
		};
	  });
...
var pullData = function(settings,callback){
	d3.json("DynamicData.json", function (err, data){
		if (err) return console.warn(err);
		flag2 = data;
		var newData = data;
		data.forEach(function(d,i){
// 			var newValue = d.value + Math.floor((Math.random()*0.0005) - 0)
			var newValue = d.value + Math.random()*0.0005;
